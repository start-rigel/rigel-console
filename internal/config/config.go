...
type Config struct {
	...
	BuildEngineServiceToken string        `yaml:"build_engine_service_token"`
...
func Load(path string) (Config, error) {
	...
	cfg := Config{
		...
		BuildEngineServiceToken: blankFallback(os.Getenv("RIGEL_INTERNAL_SERVICE_TOKEN"), raw.BuildEngineServiceToken),
	...
	if err := validateBuildEngineServiceToken(cfg.BuildEngineServiceToken); err != nil {
		return Config{}, err
	}
	return cfg, nil
}
...
func validateBuildEngineServiceToken(token string) error {
	trimmed := strings.TrimSpace(token)
	if trimmed == "" {
		return fmt.Errorf("build_engine_service_token must not be empty")
	}
	if len(trimmed) < 24 {
		return fmt.Errorf("build_engine_service_token must be at least 24 characters")
	}
	if strings.EqualFold(trimmed, "change-me-in-production") {
		return fmt.Errorf("build_engine_service_token must not use placeholder token")
	}
	return nil
}
