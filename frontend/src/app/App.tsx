...
function partCategoryLabel(category: string): string {
...
function normalizePartName(name: string): string {
  ...
  const prefixes = ['CPU ', 'GPU ', 'MB ', 'RAM ', 'SSD ', 'HDD ', 'PSU ', 'CASE ', 'COOLER '];
...
function RecommendationPage() {
  ...
  const selectedItems = result?.build_items ?? [];
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <AdminFeatureCard
          ...
          icon={<Sparkles className="size-5 text-cyan-300" />}
          title="系统设置"
          text="配置 AI upstream、token、timeout 和型号上限。"
        />
      </div>
    </AdminScaffold>
  );
...
function AdminSettingsPage() {
  useDocumentTitle('givezj8.cn · 系统设置');
  ...
  async function loadSettings() {
    try {
      ...
      setData(resp);
      setForm({
        baseURL: resp.ai_runtime.base_url || '',
        model: resp.ai_runtime.model || '',
        timeoutSeconds: String(resp.ai_runtime.timeout_seconds || 25),
        enabled: Boolean(resp.ai_runtime.enabled),
        gatewayToken: '',
        apiToken: '',
        maxModelsPerCategory: String(resp.catalog_ai_limits.max_models_per_category || 5),
      });
      setStatus('读取成功');
    } catch (err) {
      if (err instanceof APIError && err.status === 401) {
      ...
      setStatus(err instanceof APIError ? err.message : '读取设置失败');
    }
  }
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: UpdateSystemSettingsRequest = {
      ai_runtime: {
        ...
        timeout_seconds: Number(form.timeoutSeconds || '25'),
        enabled: form.enabled,
        gateway_token: form.gatewayToken.trim(),
        api_token: form.apiToken.trim(),
      },
      catalog_ai_limits: {
        max_models_per_category: Number(form.maxModelsPerCategory || '5'),
      },
    };
    try {
      const resp = await updateSystemSettings(payload);
      setData(resp);
      setForm((prev) => ({ ...prev, gatewayToken: '', apiToken: '' }));
      setStatus('保存成功');
    } catch (err) {
      setStatus(err instanceof APIError ? err.message : '保存失败');
    } finally {
  ...
  return (
    <AdminScaffold
      title="系统设置"
      description="维护 AI 运行参数和每个类别传给 AI 的型号上限。敏感字段读取时脱敏，保存时非空覆盖。"
      toolbar={
        <>
          <NavLink href="/admin">后台首页</NavLink>
          <NavLink href="/admin/keywords">词库列表</NavLink>
      ...
      <section className="rounded-[24px] border border-cyan-300/24 bg-slate-950/84 p-6 shadow-[0_18px_56px_rgba(0,0,0,0.22),inset_0_0_0_1px_rgba(130,220,255,0.05)]">
        <form className="grid gap-4 lg:grid-cols-2" onSubmit={handleSave}>
          <Field label="AI Base URL">
            <input className={controlClassName} value={form.baseURL} onChange={(e) => setForm((p) => ({ ...p, baseURL: e.target.value }))} />
          </Field>
          <Field label="AI Model">
            <input className={controlClassName} value={form.model} onChange={(e) => setForm((p) => ({ ...p, model: e.target.value }))} />
          </Field>
          <Field label="AI Timeout (s)">
            <input className={controlClassName} type="number" min="1" value={form.timeoutSeconds} onChange={(e) => setForm((p) => ({ ...p, timeoutSeconds: e.target.value }))} />
          </Field>
          <Field label="每类型号上限">
            <input className={controlClassName} type="number" min="1" max="20" value={form.maxModelsPerCategory} onChange={(e) => setForm((p) => ({ ...p, maxModelsPerCategory: e.target.value }))} />
          </Field>
          <Field label="Gateway Token (留空不改)">
            <input className={controlClassName} value={form.gatewayToken} onChange={(e) => setForm((p) => ({ ...p, gatewayToken: e.target.value }))} placeholder={data?.ai_runtime.gateway_token_masked || ''} />
          </Field>
          <Field label="AI API Token (留空不改)">
            <input className={controlClassName} value={form.apiToken} onChange={(e) => setForm((p) => ({ ...p, apiToken: e.target.value }))} placeholder={data?.ai_runtime.api_token_masked || ''} />
          </Field>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className={secondaryButtonClassName} type="button" onClick={() => void loadSettings()}>
              刷新
            </button>
          </div>
        </form>
...
function ResultPanel({
  ...
  summaryText: string;
  selectedItems: CatalogRecommendationResponse['build_items'];
  error: string;
}) {
  if (!result) {
    return (
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 rounded-[24px] border border-cyan-300/24 bg-slate-950/82 p-6 duration-500 shadow-[0_16px_48px_rgba(0,0,0,0.26),inset_0_0_0_1px_rgba(130,220,255,0.06)]">
      ...
      <section className="mt-5 grid gap-4 sm:grid-cols-3">
        <HighlightCard
          label="估算总价"
          value={`¥${result.estimated_total.toLocaleString()}`}
        ...
        <HighlightCard
          label="价格样本"
          value={`${result.catalog_item_count ?? 0} 条`}
      ...
      <section className="mt-6">
        ...
        <div className="mt-4 space-y-3">
          {selectedItems.map((item) => (
            <article
              key={`${item.category}-${item.target_model}`}
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-slate-100">{partCategoryLabel(item.category)}</p>
                  </div>
                  <p className="mt-1 break-words text-sm text-slate-300">{normalizePartName(item.target_model)}</p>
                  <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                    <span className="rounded-full border border-cyan-300/14 bg-cyan-400/6 px-2 py-1">
                      参考价 ¥{(item.recommended_product?.price || 0).toLocaleString()}
                    </span>
                  ...
                  {item.selection_reason ? (
                    <div className="mt-2 space-y-1">
                      <p className="text-xs leading-5 text-slate-400">• {item.selection_reason}</p>
                    </div>
                  ) : null}
                </div>
                <div className="shrink-0 text-left sm:text-right">
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">当前参考价</p>
                  <p className="mt-1 text-xl font-semibold text-cyan-200">¥{(item.recommended_product?.price || 0).toLocaleString()}</p>
                </div>
              </div>
            </article>
          ))}
